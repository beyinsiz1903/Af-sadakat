...
export default function ConnectorsPage() {
  ...
  return (
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="mt-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {platforms.map(platform => {
            ...
            return (
              <Card key={platform.id} className="bg-[hsl(var(--card))] border-[hsl(var(--border))]">
                <CardContent className="p-5">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      {platform.id === 'google_business' ? <Star className="w-5 h-5 text-amber-400" /> :
                       platform.id === 'tripadvisor' ? <Award className="w-5 h-5 text-emerald-400" /> :
                       <Globe className="w-5 h-5 text-blue-400" />}
                      <h3 className="font-semibold">{platform.name}</h3>
                    </div>
                    <Badge className={isConnected ? 'bg-emerald-500/20 text-emerald-400' : 'bg-gray-500/20 text-gray-400'}>
                      {isConnected ? 'Connected' : 'Not Connected'}
                    </Badge>
                  </div>
                  <p className="text-xs text-[hsl(var(--muted-foreground))] mb-3">{platform.description}</p>
                  <div className="flex gap-2">
                    {!isConnected ? (
                      <Button size="sm" onClick={() => { setPlatformConfigOpen(platform.id); setPlatformApiKey(''); setPlatformPropertyId(''); }}>
                        <Settings className="w-3 h-3 mr-1" /> Configure
                      </Button>
                    ) : (
                      <Button size="sm" variant="outline" onClick={() => disconnectPlatform.mutate(platform.id)}>
        ...
        <CardContent className="p-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold">Email & SMS Notifications</p>
                <p className="text-xs text-[hsl(var(--muted-foreground))]">
                  Auto-notify guests via email and SMS on request status changes
                </p>
              </div>
            </div>
            <Button size="sm" onClick={() => setEmailSmsOpen(true)}>
              <Settings className="w-3 h-3 mr-1" /> Configure
            </Button>
          </div>
        </CardContent>
      ...
      {/* Platform Configure Dialog */}
      <Dialog open={!!platformConfigOpen} onOpenChange={() => setPlatformConfigOpen(null)}>
        <DialogContent className="bg-[hsl(var(--card))] border-[hsl(var(--border))]">
          <DialogHeader><DialogTitle>Configure Platform</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <Input value={platformApiKey} onChange={(e) => setPlatformApiKey(e.target.value)} placeholder="API Key" />
            <Input value={platformPropertyId} onChange={(e) => setPlatformPropertyId(e.target.value)} placeholder="Property/Location ID" />
            <Button className="w-full" onClick={() => configurePlatform.mutate({
              platformId: platformConfigOpen, data: { api_key: platformApiKey, property_id: platformPropertyId }
            })} disabled={!platformApiKey}>Connect Platform</Button>
          </div>
        </DialogContent>
      </Dialog>

      {/* Email/SMS Settings Dialog */}
      <Dialog open={emailSmsOpen} onOpenChange={setEmailSmsOpen}>
        <DialogContent className="bg-[hsl(var(--card))] border-[hsl(var(--border))] max-w-lg">
          <DialogHeader><DialogTitle>Email & SMS Settings</DialogTitle></DialogHeader>
          <div className="space-y-4 text-sm">
            <div className="p-3 rounded-lg bg-[hsl(var(--secondary))]">
              <h4 className="font-semibold mb-2">Email (SMTP)</h4>
              <div className="grid grid-cols-2 gap-2">
                <Input placeholder="SMTP Host" /><Input placeholder="Port (587)" />
                <Input placeholder="User" /><Input type="password" placeholder="Password" />
              </div>
            </div>
            <div className="p-3 rounded-lg bg-[hsl(var(--secondary))]">
              <h4 className="font-semibold mb-2">SMS Provider</h4>
              <div className="grid grid-cols-2 gap-2">
                <Input placeholder="Provider (twilio)" /><Input placeholder="From" />
                <Input placeholder="API Key" /><Input type="password" placeholder="Secret" />
              </div>
            </div>
            <p className="text-xs text-[hsl(var(--muted-foreground))]">Mock mode active when not configured.</p>
            <Button className="w-full" onClick={() => { setEmailSmsOpen(false); toast.success('Settings saved'); }}>Save</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
...
function MetaAssetsDialog({ open, onClose, slug, status }) {
  ...
  return (
            ...
            <Card key={platform.id} className="bg-[hsl(var(--card))] border-[hsl(var(--border))]">
              <CardContent className="p-5">
                ...
                {platform.last_sync_at && (
                  <p className="text-[10px] text-[hsl(var(--muted-foreground))] mt-2">Last sync: {timeAgo(platform.last_sync_at)}</p>
                )}
              </CardContent>
            </Card>
          );
    ...
    {/* Email & SMS Notification Settings */}
    <div className="mt-8">
      <h2 className="text-xl font-bold mb-4">Email & SMS Notifications</h2>
      <Card className="bg-[hsl(var(--card))] border-[hsl(var(--border))]">
        <CardContent className="p-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold">Email & SMS Notifications</p>
                <p className="text-xs text-[hsl(var(--muted-foreground))]">
                  Send automatic notifications to guests via email and SMS
                </p>
              </div>
            </div>
            <Button size="sm" onClick={() => setEmailSmsOpen(true)}>
    ...
    <Dialog open={!!platformConfigOpen} onOpenChange={() => setPlatformConfigOpen(null)}>
      <DialogContent className="bg-[hsl(var(--card))] border-[hsl(var(--border))]">
        ...
        <div className="space-y-4">
          ...
          <Input value={platformPropertyId} onChange={(e) => setPlatformPropertyId(e.target.value)} placeholder="Property/Location ID" />
          <Button className="w-full" onClick={() => configurePlatform.mutate({
            platformId: platformConfigOpen,
            data: { api_key: platformApiKey, property_id: platformPropertyId }
    ...
    <Dialog open={emailSmsOpen} onOpenChange={setEmailSmsOpen}>
      <DialogContent className="bg-[hsl(var(--card))] border-[hsl(var(--border))] max-w-lg">
        <DialogHeader>
          <DialogTitle>Email & SMS Notification Settings</DialogTitle>
        </DialogHeader>
        <div className="space-y-4 text-sm">
          <div className="p-3 rounded-lg bg-[hsl(var(--secondary))]">
            <h4 className="font-semibold mb-2 flex items-center gap-2"><Mail className="w-4 h-4" /> Email (SMTP)</h4>
            <div className="grid grid-cols-2 gap-2">
              <Input placeholder="SMTP Host" defaultValue={notifSettings?.smtp_host || ''} />
              <Input placeholder="Port (587)" defaultValue={notifSettings?.smtp_port || '587'} />
              <Input placeholder="SMTP User" />
              <Input type="password" placeholder="SMTP Password" />
              <Input className="col-span-2" placeholder="From Email" />
            </div>
          </div>
          <div className="p-3 rounded-lg bg-[hsl(var(--secondary))]">
            <h4 className="font-semibold mb-2 flex items-center gap-2"><Smartphone className="w-4 h-4" /> SMS</h4>
            <div className="grid grid-cols-2 gap-2">
              <Input placeholder="Provider (twilio)" />
          ...
          <p className="text-xs text-[hsl(var(--muted-foreground))]">
            When not configured, notifications are logged but not sent (mock mode).
          </p>
          <Button className="w-full" onClick={() => { setEmailSmsOpen(false); toast.success('Settings saved'); }}>
            Save Settings
          </Button>
        </div>
      </DialogContent>
